...
function navigate(pageId, isBack = false) {
  ...
  // Trigger wheel animation when entering main page
  if (pageId === 'page-main') {
    requestAnimationFrame(() => setTimeout(runWheelAnimation, 80));
  }
...
// Each time page-main becomes active, briefly highlight the 3 sectors in
// sequence: PP (dark blue) → AL (mid blue) → WE (light blue)
// Each highlight lasts ~280ms, total sequence ~1 second.

function runWheelAnimation() {
  const sectors = [
    { id: 'sector-pp', brightFill: '#2a5db0' },
    { id: 'sector-al', brightFill: '#1a90d4' },
    { id: 'sector-we', brightFill: '#3dc8da' },
  ];

  // Reset all to original fills first
  const originals = {
    'sector-pp': '#154284',
    'sector-al': '#0471b8',
    'sector-we': '#2aabe2',
  ...
  if (window._wheelAnimTimer) {
    window._wheelAnimTimer.forEach(t => clearTimeout(t));
  }
  window._wheelAnimTimer = [];

  // Restore all sectors immediately
  sectors.forEach(({ id }) => {
    const el = document.getElementById(id);
    if (el) el.setAttribute('fill', originals[id]);
  });

  const HIGHLIGHT_MS = 260;  // how long each sector glows
  const GAP_MS       = 60;   // gap between sectors
  const STEP         = HIGHLIGHT_MS + GAP_MS;

  sectors.forEach(({ id, brightFill }, i) => {
    // Light up
    const t1 = setTimeout(() => {
      const el = document.getElementById(id);
      if (el) {
        el.style.transition = 'fill 0.12s ease-out';
    ...
    const t2 = setTimeout(() => {
      const el = document.getElementById(id);
      if (el) {
        el.style.transition = 'fill 0.18s ease-in';
        el.setAttribute('fill', originals[id]);
      }
    }, 160 + i * STEP + HIGHLIGHT_MS);

    window._wheelAnimTimer.push(t1, t2);
  });
}
